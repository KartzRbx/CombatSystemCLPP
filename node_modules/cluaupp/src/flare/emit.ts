import type { FlareEmitResult, FlareField, FlareQuery, FlarePacket, FlareSchema, FlareType } from "./types.js";

const CLPP_TYPE: Record<FlareType, string> = {
	u8: "int",
	u16: "int",
	u32: "int",
	i8: "int",
...
const WRITE: Record<FlareType, (cursor: string, value: string) => string> = {
...
const READ: Record<FlareType, (cursor: string) => string> = {
	u8: (c) => `Flare.readu8(${c})`,
...
function params(fields: FlareField[]): string {
	return fields.map((field) => field.name).join(", ");
}

function clppParams(fields: FlareField[]): string {
	return fields.map((field) => `${CLPP_TYPE[field.type]} ${field.name}`).join(", ");
}

function writeFields(fields: FlareField[], cursor = "cursor"): string {
	if (fields.length === 0) {
...
function readFields(fields: FlareField[], cursor = "cursor"): string {
	if (fields.length === 0) {
		return "\treturn";
	}
...
function emitPacketLuau(packet: FlarePacket): string {
...
function emitQueryLuau(query: FlareQuery): string {
...
export function emitFlareLuau(schema: FlareSchema): string {
...
function emitPacketHeader(ns: string, packet: FlarePacket): string {
...
function emitQueryHeader(ns: string, query: FlareQuery): string {
...
export function emitFlareHeader(schema: FlareSchema): string {
...
export function emitFlare(schema: FlareSchema, relDir: string): FlareEmitResult {
